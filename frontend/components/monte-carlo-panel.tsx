  ...
  return (
    <section className="rounded-[2rem] border border-black bg-white p-6">
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-black/55">Monte Carlo</p>
          <h2 className="mt-2 text-2xl font-semibold">Portfolio projection curve</h2>
          <p className="mt-2 max-w-2xl text-sm text-black/65">{simulation.message}</p>
        </div>
        <div className="rounded-2xl border border-black bg-black px-5 py-4 text-white">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <article className="rounded-2xl border border-black bg-[#f8f8f8] p-4">
          <p className="text-xs uppercase tracking-[0.25em] text-black/55">Years</p>
          <p className="mt-2 text-2xl font-semibold">{simulation.params.years}</p>
        </article>
        <article className="rounded-2xl border border-black bg-[#f8f8f8] p-4">
          <p className="text-xs uppercase tracking-[0.25em] text-black/55">Simulations</p>
          <p className="mt-2 text-2xl font-semibold">{simulation.params.simulations}</p>
        </article>
        <article className="rounded-2xl border border-black bg-[#f8f8f8] p-4">
          <p className="text-xs uppercase tracking-[0.25em] text-black/55">Expected Return / Volatility</p>
      ...
      {lastPoint ? (
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <article className="rounded-2xl border border-black p-4">
            <p className="text-xs uppercase tracking-[0.25em] text-black/55">Median (P50)</p>
            <p className="mt-2 text-lg font-semibold">{formatCurrency(lastPoint.p50)}</p>
          </article>
          <article className="rounded-2xl border border-black p-4">
            <p className="text-xs uppercase tracking-[0.25em] text-black/55">Conservative (P10)</p>
            <p className="mt-2 text-lg font-semibold">{formatCurrency(lastPoint.p10)}</p>
          </article>
          <article className="rounded-2xl border border-black p-4">
            <p className="text-xs uppercase tracking-[0.25em] text-black/55">Aggressive (P90)</p>
            <p className="mt-2 text-lg font-semibold">{formatCurrency(lastPoint.p90)}</p>
          </article>
          <article className="rounded-2xl border border-black p-4">
            <p className="text-xs uppercase tracking-[0.25em] text-black/55">Prob. positive</p>
            <p className="mt-2 text-lg font-semibold">{formatPercent(lastPoint.prob_positive)}</p>
          </article>
        </div>
      ) : (
        <p className="mt-6 rounded-2xl border border-black bg-[#f5f5f5] p-4 text-sm text-black/70">No portfolio positions found yet.</p>
      )}
          ...
          <table className="min-w-full border-collapse text-left text-sm">
            ...
            <tbody>
              {simulation.timeline.map((point) => (
                <tr key={point.year} className="border-b border-black/10">
